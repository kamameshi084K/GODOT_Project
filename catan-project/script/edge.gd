...
func _input_event(_viewport, event, _shape_idx):
	if event is InputEventMouseButton and event.pressed and event.button_index == MOUSE_BUTTON_LEFT:
		...
		if main != null and main.has_method("tutorial_can_build_road"):
			if not main.tutorial_can_build_road(name):
				return
			
			if main.tutorial_mode:
				var my_id = multiplayer.get_unique_id()
				
				if main.has_method("tutorial_force_build_road"):
					main.tutorial_force_build_road(name, my_id)
